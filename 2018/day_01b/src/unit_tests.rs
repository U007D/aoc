...
fn test_case_1() {
    ...
    assert_eq!(find_first_repeating_frequency(deltas), Ok(0));
}

#[test]
fn test_case_2() {
    let deltas = values_to_results_of_string(vec![3, 3, 4, -2, -4]);
    assert_eq!(find_first_repeating_frequency(deltas), Ok(10));
}

#[test]
fn test_case_3() {
    let deltas = values_to_results_of_string(vec![-6, 3, 8, 5, -6]);
    assert_eq!(find_first_repeating_frequency(deltas), Ok(5));
}

#[test]
fn test_case_4() {
    let deltas = values_to_results_of_string(vec![7, 7, -2, -7, -4]);
    assert_eq!(find_first_repeating_frequency(deltas), Ok(14));
}
